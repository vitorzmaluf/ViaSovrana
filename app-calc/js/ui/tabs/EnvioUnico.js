...
export default class EnvioUnico extends BaseTab {
  ...
  #renderResult(s) {
    ...
    const rkg = Number(cfg.parameters?.pricePerKg || 0);
    const rkgLabel = rkg > 0 ? `R$ ${fmtN(rkg, 2)}` : 'R$/kg';
    const icmsPct = Number(cfg.parameters?.effectiveIcmsPercent || 0);
    const lpPct = Number(cfg.parameters?.presumedProfitTaxPercent || 0);
    ...
    <div class="result-box">
      ...
      <div class="grid4" style="margin-top:12px;">
        ...
        <div class="stat-card">
          ...
          <div class="stat-val" style="font-size:13px;color:var(--txM)">${fmtPct(r.tributos / r.freteBruto)}</div>
        </div>
      </div>
    ...
    <div class="dre-card">
      ...
      ${dreRow(`Carga (${r.pesoKg} kg × ${rkgLabel})`, fmt(r.composicao.carga), COLOR.muted)}
      ${dreRow('Entrega zona', fmt(r.composicao.taxaZona), COLOR.muted)}
      ${dreRow('Frete cobrado', fmt(r.freteBruto), COLOR.green)}
      ${dreRow(`(−) ICMS ${pct(icmsPct)} (crédito outorgado)`, fmt(r.icms), COLOR.orange)}
      ${dreRow(`(−) LP ${pct(lpPct)}`, fmt(r.lp), COLOR.orange)}
